...
namespace Orchard.Projections.Controllers {
    ...
    public class LayoutController : Controller {
        ...
        public ActionResult Create(int id, string category, string type) {
            ...
            var layout = _projectionManager.DescribeLayouts().SelectMany(x => x.Descriptors).Where(x => x.Category == category && x.Type == type).FirstOrDefault();
            ...
            var form = _formManager.Build(layout.Form);
        ...
        public ActionResult CreatePost(LayoutEditViewModel model, FormCollection formCollection) {
            ...
            model.Layout = _projectionManager.DescribeLayouts().SelectMany(x => x.Descriptors).Where(x => x.Category == model.Category && x.Type == model.Type).FirstOrDefault();

            _formManager.Validate(new ValidatingContext { FormName = model.Layout.Form, ModelState = ModelState, ValueProvider = ValueProvider });

            var form = _formManager.Build(model.Layout.Form);
        ...
        public ActionResult Edit(int id) {
            ...
            var layoutDescriptor = _projectionManager.DescribeLayouts().SelectMany(x => x.Descriptors).Where(x => x.Category == layoutRecord.Category && x.Type == layoutRecord.Type).FirstOrDefault();

            // build the form, and let external components alter it
            var form = _formManager.Build(layoutDescriptor.Form);
            ...
            foreach (var field in layoutRecord.Properties) {
                ...
                var f = allFields.Where(x => fieldCategory == x.Category && fieldType == x.Type).FirstOrDefault();
                if (f != null) {
                    fieldEntries.Add(
                        new PropertyEntry {
                            Category = f.Category,
